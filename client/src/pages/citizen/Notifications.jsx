...
export default function Notifications() {
  ...
  async function fetchNotifications() {
    try {
      const res = await api.get("/warga/notifications");
      setList(res.data);
    } catch {
  ...
  async function markAsRead(id) {
    try {
      await api.put(`/warga/notifications/${id}/read`);
      setList((prev) =>
  ...
  async function markAllAsRead() {
    try {
      await api.put("/warga/notifications/read-all");
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Pemberitahuan dan informasi pengajuan Anda</p>
        </div>
