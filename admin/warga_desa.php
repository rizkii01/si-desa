...
if (isset($_GET['hapus'])) {
    $id = (int)$_GET['hapus'];
    $stmt = $pdo->prepare("DELETE FROM warga WHERE id = ? AND role = 'warga'"); // hanya warga, bukan admin
    $stmt->execute([$id]);
    header('Location: warga_desa.php');
    exit;
}

// Ambil semua data warga (kecuali admin)
$stmt = $pdo->prepare("SELECT * FROM warga WHERE role = 'warga' ORDER BY nama_lengkap ASC");
$stmt->execute();
...
<head>
    ...
    <title>Warga Desa - Admin Desa Cemara</title>
    ...
    <div class="d-flex" id="wrapper">
        ...
        <div class="bg-dark text-white vh-100 position-fixed" style="width: 280px;">
            ...
            <ul class="nav flex-column px-3">
                ...
                <li class="nav-item"><a href="warga_desa.php" class="nav-link text-white bg-primary py-3 px-4 rounded active"><i class="bi bi-people me-3"></i> Warga Desa</a></li>
        ...
        <div class="flex-grow-1" style="margin-left: 280px;">
            <nav class="navbar navbar-light bg-white shadow-sm px-4 py-3">
                <span class="navbar-brand h5 mb-0">Warga Desa</span>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item text-muted">Admin</li>
                    <li class="breadcrumb-item active">Warga Desa</li>
                </ol>
            </nav>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle">
                                ...
                                <tbody>
                                    <?php if (empty($wargas)): ?>
                                        <tr>
                                            <td colspan="8" class="text-center text-muted py-5">Belum ada data warga</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($wargas as $i => $w): ?>
                                            <tr>
                                                ...
                                                <td><?= $w['jenis_kelamin'] === 'Laki-laki' ? 'Laki-laki' : 'Perempuan' ?></td>
                                                <td><?= date('d-m-Y', strtotime($w['tanggal_lahir'])) ?></td>
                                                <td>
                                                    <a href="detail_warga.php?id=<?= $w['id'] ?>" class="btn btn-sm btn-info text-white"><i class="bi bi-eye"></i> Detail</a>
                                                    <a href="edit_warga.php?id=<?= $w['id'] ?>" class="btn btn-sm btn-warning text-white"><i class="bi bi-pencil"></i> Edit</a>
                                                    <a href="?hapus=<?= $w['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin hapus data <?= htmlspecialchars($w['nama_lengkap']) ?>?')">
            ...
            <?php if (isset($_GET['success'])): ?>
                <div class="alert alert-success alert-dismissible fade show">
                    <i class="bi bi-check-circle me-2"></i>Data warga berhasil ditambahkan!
            ...
            <div class="modal-content">
                <form action="proses_tambah_warga.php" method="POST">
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title"><i class="bi bi-person-plus me-2"></i>Tambah Warga Desa</h5>
